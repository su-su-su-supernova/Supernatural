...
namespace OculusXR
{
	...
	const void* FEnvironmentDepthExtensionPlugin::OnBeginSession(XrSession InSession, const void* InNext)
	{
		// Create stage space
		...
		Info.next = nullptr;
#if PLATFORM_ANDROID
		Info.referenceSpaceType = XR_REFERENCE_SPACE_TYPE_LOCAL_FLOOR_OCULUS;
#else
		Info.referenceSpaceType = XR_REFERENCE_SPACE_TYPE_STAGE;
#endif
		Info.poseInReferenceSpace = ToXrPose(FTransform::Identity);

		XR_ENSURE(xrCreateReferenceSpace(Session, &Info, &StageSpace));
