...
void FMetaXRSES::LaunchEnvironment(int32 EnvironmentIndex)
{
	...
	if (GetMetaXRSimPackagePath().IsEmpty() || SESPath.IsEmpty() || EnvironmentName.IsEmpty())
	{
		return;
	}
	StopServer();
...
FString FMetaXRSES::GetSynthEnvRootPath()
{
	FString DirPath = GetMutableDefault<UOculusXRHMDRuntimeSettings>()->SynthEnvDirPath.Path;
	if (DirPath.IsEmpty() || !IFileManager::Get().DirectoryExists(*DirPath))
	{
		FString Message("Synthetic Environment directory Not Found.\nPlease set its path in Project Settings / Meta XR Plugin / PC.");
		UE_LOG(LogMetaXRSES, Error, TEXT("%s"), *Message);
		return FString();
	}
	return DirPath;
...
TArray<FMetaXRSES::ServerInfo>& FMetaXRSES::GetSynthEnvRooms()
{
	...
	FString dirPath = GetSynthEnvRootPath();
	if (dirPath.IsEmpty())
	{
		return SynthEnvRooms;
	}
...
{
	FString Path = GetMetaXRSimPackagePath() + "/local_sharing_server~/local_sharing_server.exe";

	if (!IFileManager::Get().FileExists(*Path))
	{
		UE_LOG(LogMetaXRSES, Warning, TEXT("Failed to find %s, trying the previous version"), *Path);
		Path = GetMetaXRSimPackagePath() + "/.local_sharing_server/local_sharing_server.exe";
